...
<label for="seopress_404_enable_mails">
    ...
    <?php _e('1 email per week with the top 404 errors, and the latest logged (within a limit of 10).', 'wp-seopress-pro'); ?>
</label>
...
function seopress_404_enable_mails_from_callback() {
    ...
    ?>
        <p class="description">
            <?php _e('Separate emails by comma','wp-seopress-pro'); ?>
        </p>
    <?php
}
...
<label for="seopress_404_disable_automatic_redirects">
    ...
    <?php _e('Disable notifications on slug changes or post / term deletions', 'wp-seopress-pro'); ?>
</label>
...
<div class="postbox section-tool">
    ...
    <select id="select-wizard-redirects" name="select-wizard-redirects">
        ...
        <option value="section-import-rk-redirects"><?php _e('Rank Math plugin (JSON)', 'wp-seopress-pro'); ?>
        </option>
    </select>
    ...
    <div class="inside">
        ...
        <div class="seopress-notice">
            <p>
                <?php
                    ...
                    printf(__('Import your own redirections from a .csv file (separator %1$s or %2$s). <br>You must have these columns in this order:', 'wp-seopress-pro'), '<code>,</code>', '<code>;</code>');
                ?>
            </p>
            ...
            <ul>
                <li>
                    <?php
                        ...
                        printf(__('URL to match (without your domain name): %s', 'wp-seopress-pro'), '<code>category/my-blog-post-slug</code>');
                    ?>
                </li>
                <li>
                    <?php
                        /* translators: %s: <code>https://www.example.org/my-super-article</code> */
                        printf(__('URL to redirect in absolute: %s', 'wp-seopress-pro'), '<code>https://www.example.com/my-super-article</code>');
                    ?>
                </li>
                <li>
                    <?php
                        /* translators: %1$s: type of redirection, eg: 301 and so on */
                        printf(__('type of redirection: %1$s, %2$s, %3$s, %4$s or %5$s,', 'wp-seopress-pro'),  '<code>301</code>', '<code>302</code>', '<code>307</code>', '<code>410</code>', '<code>451</code>');
                    ?>
                </li>
                <li>
                    <?php
                        /* translators: %s equals to "yes" */
                        printf(__('%s to enable the redirect (leave it empty to disable the redirect)', 'wp-seopress-pro'), '<code>yes</code>');
                    ?>
                </li>
                <li>
                    <?php
                        /* translators: %1$s equals to the query parameter, eg: <br><code>exact_match</code> */
                        printf(__('the query parameter: %1$s = Exact match with all parameters, %2$s = Exclude all parameters, %3$s = Exclude all parameters and pass them to the redirection,', 'wp-seopress-pro'), '<br><code>exact_match</code>', '<br><code>without_param</code>', '<br><code>with_ignored_param</code>');
                    ?>
                </li>
                <li>
                ...
                <li>
                    <?php
                        ...
                        printf(__('category redirect IDs separated by commas (optional): %s', 'wp-seopress-pro'), '<code>738,108,134</code>');
                    ?>
                </li>
                <li>
                    <?php
                        /* translators: %s equals to "yes" */
                        printf(__('%s to enable regular expressions (leave it empty to disable this),', 'wp-seopress-pro'), '<code>yes</code>');
                    ?>
                </li>
                <li>
                    <?php
                        /* translators: %1$s equals is the connection status, eg: <code>only_logged_in</code> */
                        printf(__('and the last parameter, the connection status without the quotes: %1$s, %2$s or %3$s.', 'wp-seopress-pro'), '<br><code>both</code>', '<br><code>only_logged_in</code>', '<br><code>only_not_logged_in</code>');
                    ?>
                </li>
            </ul>
        ...
        <form method="post" enctype="multipart/form-data">
            <p>
                ...
                <label for="import_sep_comma"><?php _e('Comma separator: <code>,</code>', 'wp-seopress-pro'); ?></label>
            </p>
            <p>
                <input id="import_sep_semicolon" name="import_sep" type="radio" value="semicolon" />
                <label for="import_sep_semicolon"><?php _e('Semicolon separator: <code>;</code>', 'wp-seopress-pro'); ?></label>
            </p>
...
<div id="section-import-rk-redirects" class="postbox section-tool">
    <div class="inside">
        ...
        <div class="seopress-notice">
            <p>
                <?php _e('Import your own redirections from a .json file generated by Rank Math.', 'wp-seopress-pro'); ?>
            </p>
            <p>
                <?php _e('To avoid conflicts, make sure there are no duplicates between your file and existing redirects.', 'wp-seopress-pro'); ?>
            </p>
...
<div id="section-export-redirects" class="postbox section-tool">
    <div class="inside">
        ...
        <form method="post">
            ...
            <?php sp_submit_button(__('Export', 'wp-seopress-pro'), 'btn btnTertiary'); ?>
        </form>
    </div><!-- .inside -->
</div><!-- .postbox -->
...
<div id="section-clean-404" class="postbox section-tool">
    <div class="inside">
        ...
        <form method="post">
            ...
            <?php sp_submit_button(__('Delete all 404', 'wp-seopress-pro'), 'btn btnTertiary is-deletable'); ?>
        </form>
    </div><!-- .inside -->
</div><!-- .postbox -->
...
<div id="section-clean-counters" class="postbox section-tool">
    <div class="inside">
        ...
        <form method="post">
            ...
            <?php sp_submit_button(__('Reset Count column', 'wp-seopress-pro'), 'btn btnTertiary'); ?>
        </form>
    </div><!-- .inside -->
</div><!-- .postbox -->
...
<div id="section-clean-redirects" class="postbox section-tool">
    <div class="inside">
        ...
        <form method="post">
            ...
            <?php sp_submit_button(__('Delete', 'wp-seopress-pro'), 'btn btnTertiary is-deletable'); ?>
        </form>
    </div><!-- .inside -->
</div><!-- .postbox -->
<?php
